...
"")
        ...
        self.check = 0
        # 点击阵列形状按钮显示图片
        self.pushButton_3.clicked.connect(lambda: draw_figure1(self.label, init=self.init_set))
        ...
        # 导入设置按钮触发
        self.fileWin.pushButton.clicked.connect(lambda: self.fileWin.loadSetting(setWin=self.setWin, init=self.init_set))
        # 导出设置按钮触发
        self.fileWin.pushButton_2.clicked.connect(lambda: self.fileWin.saveSetting(setWin=self.setWin, init=self.init_set))
        # 保存图片按钮触发
        self.fileWin.pushButton_3.clicked.connect(lambda: self.fileWin.save_one_figure(num=self.check, init=self.init_set))
        # 保存全部图片按钮触发
